...
function getStatusClass(status) {
  ...
  if (["success", "completed", "delivered"].includes(s)) return "green";

  if (["failed"].includes(s)) return "red";

  if (["pending", "pending_vendor_balance"].includes(s)) return "yellow";

  if (["processing", "sent"].includes(s)) return "orange";

  if (["retrying", "queued"].includes(s)) return "blue";
  ...
  try {
    ...
    orders.forEach(order => {
      ...
      row.innerHTML = `
        ...
        <td class="${getStatusClass(order.orderStatus)}">
          ${order.orderStatus}
        </td>
...
// ==========================
// AUTO REFRESH (SAFE MODE)
// ==========================
setInterval(() => {
  loadOrders();
  loadStats();
}, 8000); // slightly reduced pressure (better for Render)
